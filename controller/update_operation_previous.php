...
if ($_SESSION["actual-id-update"] > 1) { 
  ...
  $previousStatus = $coursesStatus[$actualIdPos - 1];

  if ($nItemsInArray > 1) {
    header("Location: ../views/update_view.php?id=$previousId&title=$previousTitle&description=$previousDescription&status=$previousStatus");
  } else {
  ...
  header("Location: ../views/update_view.php?id=" . $_SESSION["actual-id-update"] . 
  "&title=".$_SESSION["actual-title-update"]."&description=".$_SESSION["actual-description-update"]."&status=".$_SESSION["actual-status-update"]);
}
?>